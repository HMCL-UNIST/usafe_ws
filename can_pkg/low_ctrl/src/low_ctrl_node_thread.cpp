...
private:
    ...
    ros::Subscriber local_fake_sub_;
...
public:
    LowCtrlNode() {
        ...
        aeb_sub_ = nh_.subscribe("aeb_flag", 10, &LowCtrlNode::AebFlagCallback, this);
        local_fake_sub_ = nh_.subscribe("local_fake_can", 10, &LowCtrlNode::LocalFakeCallback, this);
    ...
    void AebFlagCallback(const std_msgs::Int8::ConstPtr& aeb_msg) {
        std::thread([this, aeb_msg]() {
            aeb_cmd = aeb_msg->data;
            // ROS_INFO("aeb_cmd: %i", aeb_cmd);
        }).detach();
    }

    void LocalFakeCallback(const std_msgs::Int8::ConstPtr& loc_msg) {
        std::thread([this, loc_msg]() {
            loc_cmd = loc_msg->data;
            // ROS_INFO("loc_cmd: %i", loc_cmd);
            if (loc_cmd == 1) {
                signal_feed.data = 2;
                pg_cmd_.publish(signal_feed);
            }
    ...
    void initCanSetup() {
        ...
        can_setup_1.data[6] = 0b01100100; // 1G
        alive_cnt();
    ...
    void alive_cnt() {
        ...
        if (aeb_cmd == 1 || loc_cmd == 2) {
            can_setup_1.data[2] = 0b01000000;
        }
        else {
            can_setup_1.data[2] = 0b00000001;
        }
