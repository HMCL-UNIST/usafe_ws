...
void PointPillarsROS::createROSPubSub()
{
  ...
  point_pillar_visualizer = nh_.advertise<visualization_msgs::MarkerArray>("/point_pillar_visualizer", 1);
}
...
void PointPillarsROS::pubDetectedObject(const std::vector<float>& detections, const std_msgs::Header& in_header)
{
  ...
  for (size_t i = 0; i < num_objects; i++)
  {
    ...
    object.label = "car";
    //****
    object.space_frame = in_header.frame_id;
    geometry_msgs::Point32 _point;
    visualization_msgs::Marker marker;
    marker.header = in_header;
    marker.ns = "pp";
    marker.id = i; //Maybe should be changed to track obstacle , or maintain it regardless of tracking
    ...
    _point.x = object.pose.position.x + (object.dimensions.x/2);
    _point.y = object.pose.position.y + (object.dimensions.y/2);
    _point.z = object.pose.position.z + (object.dimensions.z/2);
    object.convex_hull.polygon.points.push_back(_point);
    ...
    _point.z = object.pose.position.z - (object.dimensions.z/2);
    object.convex_hull.polygon.points.push_back(_point);
    object.convex_hull.header.frame_id = in_header.frame_id;
  // ***

    objects.objects.push_back(object);
    markers.markers.push_back(marker);
  }
  point_pillar_visualizer.publish(markers);
