...
namespace darknet
{
    ...
    void Yolo3Detector::load(std::string& in_model_file, std::string& in_trained_file, double in_min_confidence, double in_nms_threshold, int gpu_device_id)
    {
        min_confidence_ = in_min_confidence;
        nms_threshold_ = in_nms_threshold;
        darknet_network_ = parse_network_cfg(&in_model_file[0]);

        darknet_network_->gpu_index = gpu_device_id; // Allocate specific gpu
...
void Yolo3DetectorNode::convert_rect_to_image_obj(std::vector< RectClassScore<float> >& in_objects, autoware_msgs::DetectedObjectArray& out_message,const sensor_msgs::ImageConstPtr& in_image)
{
    ...
    {
        {
            ...
            {   
                ...
                if (in_objects[i].class_type == 9) // traffic light
                {
                    obj.label = "tl";
                    
                    // publish traffic light image
                    cv_bridge::CvImagePtr cv_image = cv_bridge::toCvCopy(in_image, sensor_msgs::image_encodings::BGR8);
                    cv::Mat frame_ = cv_image->image.clone();
                    cv::Mat roi = frame_( cv::Rect(obj.x, obj.y, obj.width, obj.height) );
                    sensor_msgs::ImagePtr traffic_light_img = cv_bridge::CvImage(in_image->header, "bgr8", roi).toImageMsg();
                    traffic_light_publisher.publish(traffic_light_img);
...
void Yolo3DetectorNode::image_callback(const sensor_msgs::ImageConstPtr& in_image_message)
{
    ...
    convert_rect_to_image_obj(detections, output_message, in_image_message);
...
void Yolo3DetectorNode::Run()
{
    ...
    if (private_node_handle.getParam("gpu_device_id", gpu_device_id))
    {
        ROS_INFO("Setting gpu to  %d", gpu_device_id);
    }
    else
    {
        ROS_INFO("Gpu device id is not given, set to 0");
        gpu_device_id = 0;
    }
    ...
    yolo_detector_.load(network_definition_file, pretrained_model_file, score_threshold_, nms_threshold_, gpu_device_id);
    ...
    traffic_light_publisher = node_handle_.advertise<sensor_msgs::Image>("traffic_light_image", 1);
