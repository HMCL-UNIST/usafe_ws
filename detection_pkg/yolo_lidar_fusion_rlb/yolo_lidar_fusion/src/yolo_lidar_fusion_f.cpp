...
void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
    ...
    if (!camera_lidar_tf_ok_f_ && camera_info_ok_f_)
    {
        // Define the translation vector (x, y, z)
        // tf::Transform inverse_tf_f_;
        // tf::Vector3 translation(-0.029285511, -0.12625484, 0.0487722533);

        // // Define the rotation as a quaternion (x, y, z, w)
        // // tf::Quaternion rotation(0.5249281, -0.5199288, 0.4679359, -0.4849335);
        // tf::Quaternion rotation(-0.5199288, 0.4679359, -0.4849335, 0.5249281);  // reordered from wxyz to xyzw
    Eigen::Vector3d translation(-0.029285511, -0.12625484, 0.0487722533);

    // Define the rotation as an Eigen::Quaterniond
    Eigen::Quaterniond rotation(0.5249281, -0.5199288, 0.4679359, -0.4849335);  // w, x, y, z
    Eigen::Matrix3d rotation_matrix = rotation.toRotationMatrix();
    // Inverse the rotation quaternion
    Eigen::Matrix3d inverse_rotation_matrix = rotation_matrix.transpose();    
    Eigen::Vector3d inv_translation = -inverse_rotation_matrix*translation;
    Eigen::Quaterniond inverse_rotation(inverse_rotation_matrix);
    // Set the origin (translation) of the tf::Transform object using the Eigen vector
    // camera_lidar_tf_f_.setOrigin(tf::Vector3(inv_translation.x(), inv_translation.y(), inv_translation.z()));
    // camera_lidar_tf_f_.setRotation(tf::Quaternion(inverse_rotation.x(), inverse_rotation.y(), inverse_rotation.z(), inverse_rotation.w()));


    camera_lidar_tf_f_.setOrigin(tf::Vector3(-0.125, 0.047, 0.036));
    camera_lidar_tf_f_.setRotation(tf::Quaternion(0.525, -0.520, 0.468, 0.485));

    // camera_lidar_tf_f_.setOrigin(tf::Vector3(-0.029, -0.126, 0.049));
    // camera_lidar_tf_f_.setRotation(tf::Quaternion(0.525, -0.520, 0.468, -0.485));

        // Create a Transform object
        // // tf::Transform camera_lidar_tf_f_;
        // camera_lidar_tf_f_.setOrigin(translation);
        // camera_lidar_tf_f_.setRotation(rotation);    
        // camera_lidar_tf_f_ = inverse_tf_f_.inverse();
        // camera_lidar_tf_f_.inverse();
        camera_lidar_tf_ok_f_ = true;
    ...
    if (!camera_info_ok_f_ || !camera_lidar_tf_ok_f_)
    {
    ...
    for (size_t i = 0; i < transformed_cloud_f->points.size(); i++){
        ...
        if (transformed_cloud_f->points[i].z > 0){
    ...
    combined_filtered_clouds.reserve(filtered_clouds_f.size());
    ...
    for (size_t i = 0; i < combined_filtered_clouds.size(); i++){
        ...
        if (min_cluster_index >= 0) {
            ...
            Box bbox = axisAlignedBoundingBox(closest_cluster, i, x_roi_threshold);
    ...
    bbox_header.stamp = ros::Time::now();
...
ROSPixelCloudFusionApp::FindTransform(const std::string &in_target_frame, const std::string &in_source_frame)
{
    ...
    std::cout << in_target_frame << std::endl;
    std::cout << in_source_frame << std::endl;
    try
    {
        ...
        if (in_target_frame == image_frame_id_f_)
        {
            camera_lidar_tf_f_ = transform;           // Store transform for the left camera
            camera_lidar_tf_ok_f_ = true;             // Mark left transform as available
...
void ROSPixelCloudFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
    ...
    in_private_handle.param<float>("x_roi_threshold", x_roi_threshold, 40.0);
