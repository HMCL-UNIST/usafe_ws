...
from livoxdetection.models.ld_base_v2 import LD_base
...
from vis_ros import ROS_MODULE_OUSTER
ros_vis = ROS_MODULE_OUSTER()
last_box_num = 0
last_gtbox_num = 0
...
# def end_timer(start_time, task_name):
#     global count
#     end_time = time.time()
#     elapsed_time = end_time - start_time
#     #count = count + 1
#     print(f"{task_name} 소요 시간: {elapsed_time} 초, count = {count}")
...
class ros_demo():
    def __init__(self, model, args=None):
        ...
        self.point_cloud_range = [-10, -12.8, -2, 10, 12.8, 4]
        # self.point_cloud_range = [0, -19.2, -2, 224, 19.2, 4] 
        # self.point_cloud_range = [-112, -44.8, -2, 112, 44.8, 4]
        # self.point_cloud_range = [-112, -12.8, -2, 112, 12.8, 4]
 
        # -> the point cloud filter range
        # x : 0 ~ 224m range but if -224 ~ 224 -> size error occurs. need to be fixed
 
    def receive_from_ros(self, msg):
        pc = ros_numpy.numpify(msg)
        x = np.float32(pc['x']).flatten()
        y = np.float32(pc['y']).flatten()
        z = np.float32(pc['z']).flatten()
        intensity = np.float32(pc['intensity']).flatten()

        points_list = np.zeros((len(x), 4))
        points_list[:, 0] = copy.deepcopy(x)
        points_list[:, 1] = copy.deepcopy(y)
        points_list[:, 2] = copy.deepcopy(z)
        points_list[:, 3] = copy.deepcopy(intensity)
 
        # preprocess
        points_list[:, 2] += points_list[:, 0] * np.tan(self.offset_angle / 180. * np.pi) + self.offset_ground
        # -> to set ground's z coordinate to 0 ? / offset_ground -> vehicle's height(livox z position)
    ...
    def online_inference(self, msg):
        ...
        delay = (current_time - msg.header.stamp).to_sec() * 1000
        if delay > 200:  # If delay is greater than 500ms, skip the processing
        ...
        data_infer, pred_dicts = ROS_MODULE_OUSTER.gpu2cpu(data_infer, pred_dicts)
...
if __name__ == '__main__':
    ...
    # checkpoint = torch.load(args.pt, map_location=torch.device('cuda:0'))
    ...
    sub = rospy.Subscriber(
       "/ouster/points", PointCloud2, queue_size=3, callback=demo_ros.online_inference)
    # sub = rospy.Subscriber(
        # "/points_concat", PointCloud2, queue_size=3, callback=demo_ros.online_inference)
    # sub = rospy.Subscriber(
    #     "/livox/lidar_192_168_20_170", PointCloud2, queue_size=3, callback=demo_ros.online_inference)
