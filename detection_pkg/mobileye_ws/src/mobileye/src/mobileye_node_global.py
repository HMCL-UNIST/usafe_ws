...
from geometry_msgs.msg import PoseStamped, Pose, PoseWithCovariance, TwistStamped
...
class mobileyeSub():
    def __init__(self):
        ...
        self.objs_Pub = rospy.Publisher('/detection/mob/detected_object_array', DetectedObjectArray, queue_size=10)
        self.objs_viz_Pub = rospy.Publisher('/mobileye_detected_objs_viz', MarkerArray, queue_size=2)
        self.can_sub = rospy.Subscriber("/mobileye_received_messages", Frame, self.data_pub)
        # self.poseSub = rospy.Subscriber("/pose_estimate", Odometry, self.poseCallback)
        self.poseSub = rospy.Subscriber("/current_pose", PoseStamped, self.poseCallback)
        self.velSub = rospy.Subscriber("/current_velocity", TwistStamped, self.velCallback)
    ...
    def detected_obj_pub_callback(self):
        ...
        objs_msg.header.frame_id = "livox_frame_f"
        
        for i in range(len(self.obstacle_data)):

            obj_msg  = DetectedObject()
            obj_msg.header.frame_id = "livox_frame_f"
            ...
            if self.vel_ready:
                local_vx = self.obstacle_data[i].obstacle_relative_velocity_x*math.cos(self.ego_yaw)
                local_vy = self.obstacle_data[i].obstacle_relative_velocity_x*math.sin(self.ego_yaw)
                
                global_vx = self.ego_vel.twist.linear.x + local_vx
                global_vy = self.ego_vel.twist.linear.y + local_vy

                obj_msg.velocity.linear.x = global_vx
                obj_msg.velocity.linear.y = global_vy
            ...
            if(add_obj):
                ...
                # print("vel_x : " , obj_msg.velocity.linear.x)
    ...
    def poseCallback(self,odom_msg):
        ...
        self.ego_geo_pose = odom_msg.pose
        
    def velCallback(self,vel_msg):
        self.vel_ready = True
        self.ego_vel = vel_msg
