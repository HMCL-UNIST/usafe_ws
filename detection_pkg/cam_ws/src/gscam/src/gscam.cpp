...
namespace gscam {
  ...
  bool GSCam::init_stream()
  {
    ...
    if (image_encoding_ == "jpeg") {
        ...
        // image_pub_kr = nh_.advertise<sensor_msgs::Image>("camera/image_raw",1);
        // info_pub_kr = nh_.advertise<sensor_msgs::CameraInfo>("camera/camera_info",1);
        std::cout << "work" << std::endl;
  ...
  void GSCam::publish_stream()
  {
    ...
    while(ros::ok()) 
    {
      ...
      if (image_encoding_ == "jpeg") {
          ...
          // std::cout << expected_frame_size << std::endl;
          ...
          // image_pub_kr.publish(img);
          // info_pub_kr.publish(cinfo);
      }
