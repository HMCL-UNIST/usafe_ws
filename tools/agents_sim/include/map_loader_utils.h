...
int get_closest_lanelet(lanelet::Lanelets & lanelets, geometry_msgs::Pose & pose)
{
  double closest_distance = std::numeric_limits<double>::max();
  int closest_index = 0;
  for (int i = 0; i < lanelets.size(); i++) {
    auto & llt = lanelets.at(i);
    auto & point2d =
      lanelet::Point2d(lanelet::InvalId, pose.position.x, pose.position.y)
      .basicPoint2d();    
    double distance = lanelet::geometry::distanceToCenterline2d(llt, point2d);
    if (distance < closest_distance) {
      closest_distance = distance;
      closest_index = i;
    }
...
visualization_msgs::MarkerArray viz_local_paths(hmcl_msgs::Lane &lane_){
  visualization_msgs::MarkerArray local_traj_marker_arrary;
              ...
              for(int i=0; i< lane_.waypoints.size();i++){
                ...
                return local_traj_marker_arrary;          
}