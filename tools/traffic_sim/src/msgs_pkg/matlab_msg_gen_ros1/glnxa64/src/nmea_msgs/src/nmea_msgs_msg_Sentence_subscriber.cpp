...
// Subscriber for nmea_msgs/Sentence
...
namespace Sentence {
  MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const nmea_msgs::Sentence>& msg);
} //namespace Sentence
} //namespace matlabhelper
} //namespace nmea_msgs
class NMEA_MSGS_EXPORT nmea_msgs_msg_Sentence_subscriber : public MATLABSubscriberInterface {
  ...
  public:
    ...
    void callback(const nmea_msgs::Sentence::ConstPtr& msg){
      if(mSd){
        auto outArray = nmea_msgs::matlabhelper::Sentence::get_arr(mFactory, msg);
        appendAndSendToMATLAB(mSd, mSendDataToMATLABFunc, outArray);
      }
    }
    nmea_msgs_msg_Sentence_subscriber()
        : MATLABSubscriberInterface() {
    }
    virtual ~nmea_msgs_msg_Sentence_subscriber() {
    }
    virtual intptr_t createSubscription(const std::string& topic_name,
        ...
        mSub = std::make_shared<ros::Subscriber>(
                n->subscribe<nmea_msgs::Sentence>(
                            topic_name, 
                            buffer_size, 
                            &nmea_msgs_msg_Sentence_subscriber::callback, 
                            this,
...
CLASS_LOADER_REGISTER_CLASS(nmea_msgs_msg_Sentence_subscriber, MATLABSubscriberInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
