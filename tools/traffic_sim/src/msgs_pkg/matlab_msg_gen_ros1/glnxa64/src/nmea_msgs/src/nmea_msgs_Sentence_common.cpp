...
// Common copy functions for nmea_msgs/Sentence
...
namespace Sentence {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const nmea_msgs::Sentence::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  NMEA_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<nmea_msgs::Sentence>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //sentence
        const matlab::data::CharArray sentence_arr = arr[0]["sentence"];
        msg->sentence = sentence_arr.toAscii();
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'sentence' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'sentence' is wrong type; expected a string.");
    }
  }
  //----------------------------------------------------------------------------
  NMEA_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const nmea_msgs::Sentence>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","sentence"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // sentence
    outArray[0]["sentence"] = factory.createCharArray(msg->sentence);
