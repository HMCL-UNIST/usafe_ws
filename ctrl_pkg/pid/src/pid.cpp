...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("topic_from_plant", topic_from_plant_, "CAN_VehicleStatus");
  ...
  mission_sub = node.subscribe("/mission_state", 1, &PidObject::missionCallback, this);
  ...
  while( ros::ok() && !ros::topic::waitForMessage<hmcl_msgs::VehicleStatus>(topic_from_plant_, ros::Duration(10.)))
...
void PidObject::setpointCallback(const std_msgs::Float64& setpoint_msg)
{
  ...
  }else if(setpoint_ > 150/3.6 && setpoint_ < 160/3.6){
    setpoint_ = 150/3.6;
...
void PidObject::plantStateCallback(const hmcl_msgs::VehicleStatus& state_msg)
{
  
  plant_state_ = state_msg.wheelspeed.wheel_speed ;
  new_state_or_setpt_ = true;
}
...
void PidObject::missionCallback(const std_msgs::Int16::ConstPtr& msg){
    getMission = true;
...
void PidObject::doCalcs()
{
  ...
  { 
                         ...
                         (ros::Time::now() - last_setpoint_msg_time_).toSec() <= setpoint_timeout_))
    {
      ...
      if(currentMission == MissionState::MissionComplete) control_msg_.data = -2.0;
      else control_msg_.data = control_effort_;
