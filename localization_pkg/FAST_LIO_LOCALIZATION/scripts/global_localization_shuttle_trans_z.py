...
#     # Compute normal vectors for both source and target point clouds
#     pc_scan.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
#     pc_map.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
...
def min_z(pose_estimation):
    global cur_odom, global_map
    # 当前scan原点的位姿
    ...
    # 중심에서 1m 근처의 포인트들 중에서 가장 작은 z 값 찾기
    center = np.sqrt(T_base_link_to_map[0,3] ** 2 + T_base_link_to_map[1,3] ** 2)


    distances_xy = np.abs(np.sqrt(global_map_in_map[indices[0], 0] ** 2 +global_map_in_map[indices[0], 1] ** 2) - 
                          np.sqrt(T_map_to_base_link[0, 3] ** 2 +T_map_to_base_link[1, 3] ** 2))

    # 1m 근처의 포인트들 필터링
    close_points_indices = np.where(distances_xy < 2.0)  # 1m ± 0.1m 범위 내의 포인트들 선택
    if len(close_points_indices[0]) > 0:
        min_z_value = np.min(global_map_in_map[indices[0][close_points_indices], 2])
        print(f"Minimum z value within 1m range: {min_z_value}")
    else:
        min_z_value = None
        print("No points found within 1m range.")
...
def cb_save_init_pose(init_msg):
    ...
    init_z = min_z(pose_to_mat(init_msg.pose.pose.position, init_msg.pose.pose.orientation))
    ...
    if initialized:
        ...
        init_pose = initial_pose @ inverse_se3(os2livox) @inverse_se3(cur_pose)
        init_pose[2,3] =  init_z + 1.1

        T_map_to_odom = init_pose

def cb_save_stop_init_pose(stop_init_msg):
    global stopinitialized, ini_thres
    ini_thres = 0
    if stopinitialized:
        stopinitialized = False
        rospy.logwarn('Global OFF!!!!!')
    else:
        stopinitialized = True
        rospy.logwarn('Global ON!!!!!')
...
def thread_localization():
    ...
    while initialized:
        if reinitialized:
            ...
            # T_map_to_odom = inverse_se3(cur_pose) @ T_map_to_odom
            # T_map_to_odom = T_map_to_odom @ inverse_se3(cur_pose)
            # T_map_to_odom = cur_pose @ T_map_to_odom
            # T_map_to_odom = T_map_to_odom @ inverse_se3(cur_pose)
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/stop', PoseStamped, cb_save_stop_init_pose, queue_size=1)
    ...
    while not initialized:
        ...
        # init_pose = initial_pose @ inverse_se3(cur_pose)
        init_pose = initial_pose @ inverse_se3(os2livox) @inverse_se3(cur_pose)
        init_z = min_z(pose_to_mat(pose_msg.pose.pose.position, pose_msg.pose.pose.orientation))
        init_pose[2,3] = init_z + 1.1
