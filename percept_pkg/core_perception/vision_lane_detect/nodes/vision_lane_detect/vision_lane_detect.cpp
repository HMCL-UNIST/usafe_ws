...
static ros::Publisher lane_image_publisher;
...
static void processLanes(CvSeq *lines, IplImage* edges, IplImage *temp_frame, IplImage *org_frame)
{
  ...
  sensor_msgs::ImagePtr lane_image = cv_bridge::CvImage(std_msgs::Header(),"bgr8",cv::cvarrToMat(temp_frame)).toImageMsg();
  lane_image_publisher.publish(lane_image);
}
...
static void process_image_common(IplImage *frame)
{
...
#endif
  CvSize    frame_size = cvSize(video_size.width*0.46, video_size.height*0.19);
  ...
  crop(frame, temp_frame, cvRect(video_size.width*0.27, video_size.height*0.50, frame_size.width, frame_size.height));
...
int main(int argc, char *argv[])
{
  ...
  lane_image_publisher = n.advertise<sensor_msgs::Image>("/lane_image",1);
  ros::spin();
#endif
