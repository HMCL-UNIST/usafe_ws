...
void RegionTLRSSDROSNode::ImageRawCallback(const sensor_msgs::Image& image)
{
  ...
  LightState current_state = recognizer.RecognizeLightState(frame_);
  
  //DEBUG
  if (current_state == 0){
    std::cout << "[TLR SSD] GREEN LIGHT DETECTED" << std::endl;
  }
  else if(current_state == 1){
    std::cout << "[TLR SSD] YELLOW LIGHT DETECTED" << std::endl;
  }
  else if(current_state == 2){
    std::cout << "[TLR SSD] RED LIGHT DETECTED" << std::endl;
  }
  
  std::vector<Context> tmp_contexts;
  Context tmp_context;
  tmp_context.lightState = current_state;
  tmp_contexts.push_back(tmp_context);
  PublishTrafficLight(tmp_contexts);
