...
namespace Frame {
  MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const can_msgs::Frame>& msg);
} //namespace Frame
} //namespace matlabhelper
} //namespace can_msgs
class CAN_MSGS_EXPORT can_msgs_msg_Frame_subscriber : public MATLABSubscriberInterface {
  ...
  public:
    ...
    void callback(const can_msgs::Frame::ConstPtr& msg){
      if(mSd){
        auto outArray = can_msgs::matlabhelper::Frame::get_arr(mFactory, msg);
        appendAndSendToMATLAB(mSd, mSendDataToMATLABFunc, outArray);
      }
    }
    can_msgs_msg_Frame_subscriber()
        : MATLABSubscriberInterface() {
    }
    virtual ~can_msgs_msg_Frame_subscriber() {
    }
    virtual intptr_t createSubscription(const std::string& topic_name,
        ...
        mSub = std::make_shared<ros::Subscriber>(
                n->subscribe<can_msgs::Frame>(
                            topic_name, 
                            buffer_size, 
                            &can_msgs_msg_Frame_subscriber::callback, 
                            this,
...
CLASS_LOADER_REGISTER_CLASS(can_msgs_msg_Frame_subscriber, MATLABSubscriberInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
