...
// Common copy functions for autorally_msgs/runstop
...
namespace runstop {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const autorally_msgs::runstop::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AUTORALLY_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<autorally_msgs::runstop>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //motionEnabled
        const matlab::data::TypedArray<bool> motionEnabled_arr = arr[0]["motionEnabled"];
        msg->motionEnabled = motionEnabled_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'motionEnabled' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'motionEnabled' is wrong type; expected a logical.");
    }
  }
  //----------------------------------------------------------------------------
  AUTORALLY_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const autorally_msgs::runstop>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","sender","motionEnabled"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // sender
    outArray[0]["sender"] = factory.createCharArray(msg->sender);
    // motionEnabled
    outArray[0]["motionEnabled"] = factory.createScalar(static_cast<bool>(msg->motionEnabled));
