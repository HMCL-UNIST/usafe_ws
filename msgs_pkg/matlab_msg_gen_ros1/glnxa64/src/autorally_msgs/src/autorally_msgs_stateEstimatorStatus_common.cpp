...
// Common copy functions for autorally_msgs/stateEstimatorStatus
...
namespace stateEstimatorStatus {
  void copy_from_arr_std_msgs_Header(std_msgs::Header& val, const matlab::data::StructArray& arr);
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const autorally_msgs::stateEstimatorStatus::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AUTORALLY_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<autorally_msgs::stateEstimatorStatus>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //status
        const matlab::data::TypedArray<int8_t> status_arr = arr[0]["status"];
  ...
  //----------------------------------------------------------------------------
  AUTORALLY_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const autorally_msgs::stateEstimatorStatus>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","OK","WARN","ERROR","status"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // OK
    outArray[0]["OK"] = factory.createScalar(static_cast<int8_t>(msg->OK));
    // WARN
    outArray[0]["WARN"] = factory.createScalar(static_cast<int8_t>(msg->WARN));
    // ERROR
    outArray[0]["ERROR"] = factory.createScalar(static_cast<int8_t>(msg->ERROR));
    // status
