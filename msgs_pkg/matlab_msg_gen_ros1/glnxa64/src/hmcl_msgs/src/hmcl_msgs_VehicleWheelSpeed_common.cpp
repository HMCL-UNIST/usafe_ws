...
// Common copy functions for hmcl_msgs/VehicleWheelSpeed
...
namespace VehicleWheelSpeed {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const hmcl_msgs::VehicleWheelSpeed::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  HMCL_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<hmcl_msgs::VehicleWheelSpeed>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //wheel_speed
        const matlab::data::TypedArray<double> wheel_speed_arr = arr[0]["wheel_speed"];
        msg->wheel_speed = wheel_speed_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
