...
// Common copy functions for autoware_msgs/AccelCmd
...
namespace AccelCmd {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const autoware_msgs::AccelCmd::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<autoware_msgs::AccelCmd>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //accel
        const matlab::data::TypedArray<int32_t> accel_arr = arr[0]["accel"];
        msg->accel = accel_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'accel' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'accel' is wrong type; expected a int32.");
    }
  }
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const autoware_msgs::AccelCmd>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","accel"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // accel
    outArray[0]["accel"] = factory.createScalar(msg->accel);
