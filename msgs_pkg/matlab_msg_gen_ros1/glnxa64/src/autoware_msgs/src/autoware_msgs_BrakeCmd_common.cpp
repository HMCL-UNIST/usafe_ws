...
// Common copy functions for autoware_msgs/BrakeCmd
...
namespace BrakeCmd {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const autoware_msgs::BrakeCmd::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<autoware_msgs::BrakeCmd>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //brake
        const matlab::data::TypedArray<int32_t> brake_arr = arr[0]["brake"];
        msg->brake = brake_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'brake' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'brake' is wrong type; expected a int32.");
    }
  }
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const autoware_msgs::BrakeCmd>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","brake"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // brake
    outArray[0]["brake"] = factory.createScalar(msg->brake);
