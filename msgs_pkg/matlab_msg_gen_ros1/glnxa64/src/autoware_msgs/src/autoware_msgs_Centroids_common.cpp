...
// Common copy functions for autoware_msgs/Centroids
...
namespace Centroids {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const autoware_msgs::Centroids::_header_type& val);
  ...
  MDArray_T get_arr_geometry_msgs_Point_var_arr(MDFactory_T& factory, const autoware_msgs::Centroids::_points_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_std_msgs_Header(std_msgs::Header& val, const matlab::data::StructArray& arr) {
  ...
  void copy_from_arr_geometry_msgs_Point_var_arr(geometry_msgs::Point& val, const matlab::data::Struct& arr) {
    ...
    try {
        const matlab::data::TypedArray<double> _pointsx_arr = arr["x"];
    ...
    try {
        const matlab::data::TypedArray<double> _pointsy_arr = arr["y"];
    ...
    try {
        const matlab::data::TypedArray<double> _pointsz_arr = arr["z"];
        val.z = _pointsz_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  MDArray_T get_arr_geometry_msgs_Point_var_arr(MDFactory_T& factory, const autoware_msgs::Centroids::_points_type& val) {
    auto _pointsoutArray = factory.createStructArray({1,val.size()},{"x","y","z"});
    ...
    // _geometry_msgs_Point_var_arr.z
    	_pointsoutArray[idx]["z"] = factory.createScalar(val[idx].z);
    }
    return std::move(_pointsoutArray);
  }
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<autoware_msgs::Centroids>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        ...
        for (auto _pointsarr : points_arr) {
        	...
        	copy_from_arr_geometry_msgs_Point_var_arr(_val,_pointsarr);
  ...
  //----------------------------------------------------------------------------
  AUTOWARE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const autoware_msgs::Centroids>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","points"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // points
    outArray[0]["points"] = get_arr_geometry_msgs_Point_var_arr(factory, msg->points);
