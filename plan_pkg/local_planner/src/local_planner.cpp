    ...
    nh_(nh)
{
    ...
    timer_ = nh_.createTimer(ros::Duration(1), &localplanner::connect_handler,this);
...
void localplanner::connect_handler(const ros::TimerEvent& time){
    ...
    if(range_index.size() == 0){
        ...
        ROS_INFO("One Lane");
        lane_id = 1;
        // int startidx = FindClosest(range_lane.lanes[lane_id], current_pos.section);
        endidx = FindIndex(range_lane.lanes[lane_id], lc_dist, current_pos.section);
        Eigen::Vector3d pt_e(range_lane.lanes[lane_id].waypoints[endidx].pose.pose.position.x, range_lane.lanes[lane_id].waypoints[endidx].pose.pose.position.y, 0);
        ...
        X.push_back(pt_e[0]/nn);
        Y.push_back(pt_e[1]/nn);

        X.push_back(pt_e[0]- pt_e[0]/nn);
        ...
        lane_id =1;
        endidx = FindIndex(range_lane.lanes[lane_id], lc_dist +lc_prepare_dist+lane_width, current_pos.section);
        Eigen::Vector3d pt_e(range_lane.lanes[lane_id].waypoints[endidx].pose.pose.position.x, range_lane.lanes[lane_id].waypoints[endidx].pose.pose.position.y, 0);
        pt_e = tf3x3.inverse()*(pt_e - t);
        int nn = abs(int(pt_e[0]));
    ...
    for (int j = endidx+1; j <range_lane.lanes[lane_id].waypoints.size(); j++){
        hmcl_msgs::Waypoint wpt;
        wpt.pose.pose.position.x = range_lane.lanes[lane_id].waypoints[j].pose.pose.position.x;
        wpt.pose.pose.position.y = range_lane.lanes[lane_id].waypoints[j].pose.pose.position.y;
        wpt.pose.pose.position.z = 0;
        
        wpt.pose.pose.orientation.x = range_lane.lanes[lane_id].waypoints[j].pose.pose.orientation.x;
        wpt.pose.pose.orientation.y = range_lane.lanes[lane_id].waypoints[j].pose.pose.orientation.y;
        wpt.pose.pose.orientation.z = range_lane.lanes[lane_id].waypoints[j].pose.pose.orientation.z;
        wpt.pose.pose.orientation.w = range_lane.lanes[lane_id].waypoints[j].pose.pose.orientation.w;
        pub_lane.waypoints.push_back(wpt);
    }
    viz_local(pub_lane);
