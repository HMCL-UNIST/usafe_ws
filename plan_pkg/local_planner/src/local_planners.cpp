...
int localplanners::FindIndex(const hmcl_msgs::Lane& lane, double dist, geometry_msgs::Pose pose){
    ...
    for (int i =idx+1; i <lane.waypoints.size(); i++){
        x_a = lane.waypoints[i-1].pose.pose.position.x;
        y_a = lane.waypoints[i-1].pose.pose.position.y;

        x_b = lane.waypoints[i].pose.pose.position.x;
        y_b = lane.waypoints[i].pose.pose.position.y;


        dis_ = dis_ + sqrt(pow(x_a- x_b,2) + pow(y_a -y_b,2));
        if ( dis_< dist){
            idx = i+1;
            max_idx = true;
            }
    ...
    if(max_idx){
        ...
        for (int i =1; i <lane.waypoints.size(); i++){
            x_a = lane.waypoints[i-1].pose.pose.position.x;
            y_a = lane.waypoints[i-1].pose.pose.position.y;
