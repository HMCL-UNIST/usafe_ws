...
std::vector<hmcl_msgs::Waypoint> generateHermiteCurveForROS(const geometry_msgs::Pose &start,
                                                                const geometry_msgs::Pose &end,
                                                                const double velocity_mps, const double vlength)
{
  std::vector<hmcl_msgs::Waypoint> wps;
                                            ...
                                            const Element2D &v1, const double vlength)
{
  ...
  while (divide < loop)
  {
    ...
    for (int32_t i = 0; i < divide; i++)
    {
      ...
      double x = (p0.x * coeff_p0 + vlength * v0.x * coeff_v0 + p1.x * coeff_p1 + vlength * v1.x * coeff_v1);
      double y = (p0.y * coeff_p0 + vlength * v0.y * coeff_v0 + p1.y * coeff_p1 + vlength * v1.y * coeff_v1);

      // 디버깅 정보 출력
      // std::cout << "u: " << u << ", u^2: " << u_square << ", u^3: " << u_cube << std::endl;
      // std::cout << "coeff_p0: " << coeff_p0 << ", coeff_v0: " << coeff_v0 << ", coeff_p1: " << coeff_p1 << ", coeff_v1: " << coeff_v1 << std::endl;
      // std::cout << "x: " << x << ", y: " << y << std::endl;
      // if (std::isnan(x) || std::isnan(y)) {
      //   std::cout << "NaN detected: x = " << x << ", y = " << y << std::endl;
      // }

      result.push_back(Element2D(x, y));
    }
