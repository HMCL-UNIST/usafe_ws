...
BehaviorPlanner::BehaviorPlanner(){
    ...
    nh_.param<float>("wLane", wLane, 2.8);
    ...
    objs_sub = nh_.subscribe("/tracking_side/objects",1, &BehaviorPlanner::objsCallback,this);
...
void BehaviorPlanner::updateFactors(){
    ...
    if (externalMission >= 4 && externalMission <= 8){
        goToSecondLane = true;
    }
    ...
    float dist_pit = distance(egoPose.position.x,egoPose.position.y,pit_x,pit_y);
    if(currentMission == MissionState::PitStop && dist_pit < thresPit) missionEnd = true;
    updatePreferLaneID();
    getCurrentLaneWpt();
    ...
    psarr = sarr0;
    this->calculateLon(n0, psarr, globalLaneArray.lanes[lane_id]);

    // calculate Frenet coordinate of ego vehicle
    psl = slEgo;
    this->calculateFrenet(n0, psarr, egoPose.position.x, egoPose.position.y, 0, 0, psl, globalLaneArray.lanes[lane_id]);
    sEgo = slEgo[0];
    lEgo = slEgo[1];
    dsEgo = egoSpeed;
    ...
    for(int i = 0; i < on; i++){
        psarr = sarr0;
        this->calculateFrenet(n0, psarr, xObj[i], yObj[i], 0, 0, psl, globalLaneArray.lanes[lane_id]);
        sObj[i] = slObj[0];
        lObj[i] = slObj[1];
        dsObj[i] = velObj[i];
        dlObj[i] = 0;
        ...
        ROS_INFO("i:%d,sObj: %f,lObj: %f,velObj: %f, label: %d",i,sObj[i],lObj[i],velObj[i],isCar);
        if(abs(lObj[i]) <= wLane/2){
            // front vehicle
            if(sObj[i]-sEgo < minFront && sObj[i]-sEgo > 5.0){
                front_id = i;
            ...
            if(sObj[i]-sEgo < minLeft && sObj[i]-sEgo > 0.0){
                left_id = i;
                minLeft = sObj[i]-sEgo;
                leftOpponent = true;
            }
            ...
            if(sObj[i]-sEgo < minRight && sObj[i]-sEgo > 0.0){
                right_id = i;
    ...
    if(currentBehavior == BehaviorState::LeftLaneChange && prev_lane_id == lane_id && abs(lObs)<=wLane/2 && sObs-sEgo>=0.0){
        needLeftLaneChange = true;
    }
    else if(currentBehavior == BehaviorState::LeftLaneChange && prev_lane_id != lane_id && abs(lEgo)>0.8){
        needLeftLaneChange = true;
    }
    else{
    }
    ...
    if(currentBehavior == BehaviorState::RightLaneChange && prev_lane_id == lane_id && abs(lObs)<=wLane/2 && sObs-sEgo>=0.0){
        needRightLaneChange = true; 
    } 
    else if(currentBehavior == BehaviorState::RightLaneChange && prev_lane_id != lane_id && abs(lEgo)>0.8){
        needRightLaneChange = true;
    }
    ...
    if(frontOpponent){
        if(lane_id > 1) needLeftLaneChange = true;
        if(lane_id <= 1) needRightLaneChange = true;
        prev_lane_id = lane_id;
        ROS_INFO("111");
    }
...
void BehaviorPlanner::updateBehaviorState(){
    ...
    updateFactors();
    bool goToStraight = false;
    bool goToBank = false;
    BehaviorState prevBehavior = currentBehavior;
    if(prevBehavior == BehaviorState::BehaviorInit){
        if(vehicleReady){
    ...
    else if(prevStraight){
        if(isInBank){
            goToBank = true;
        }
        else if(isBankClose){
            currentBehavior = BehaviorState::ReadyForBank;
        }
        else{
            goToStraight = true;
        }
    ...
    if(goToStraight){
        if(needLeftLaneChange){
            ...
            currentBehavior = BehaviorState::StraightFreeDrive;
        }
    }
    if(goToBank){
        if(frontOpponent){
            currentBehavior = BehaviorState::BankACC;
        }
        else{
            currentBehavior = BehaviorState::BankFreeDrive;
        }
    }

    prevStraight = false;

    if(currentBehavior == BehaviorState::StraightFreeDrive || currentBehavior == BehaviorState::LeftLaneChange 
        || currentBehavior == BehaviorState::RightLaneChange || currentBehavior == BehaviorState::ReadyForBank){
        prevStraight = true;
    }
    else if(currentBehavior == BehaviorState::BankFreeDrive || currentBehavior == BehaviorState::BankACC){
        prevBank = true;
    }
